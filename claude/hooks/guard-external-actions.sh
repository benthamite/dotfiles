#!/bin/bash
# PreToolUse hook: approval-by-default guard for MCP tools (Claude Code copy).
#
...
#   Claude Code      exit 0 + permissionDecision ask/deny (this copy)
...
tool_input_json() {
  printf '%s' "$INPUT" | jq -c '
    ...
    if ($input | type) == "object" then $input
      ...
      ((try ($input | fromjson) catch {}) | if type == "object" then . else {} end)
    else {}
    end'
}
...
emit_needs_approval() {
  ...
  jq -n --arg label "$label" --arg detail "$detail" '{
    "hookSpecificOutput": {
      ...
      "permissionDecisionReason": ("NEEDS APPROVAL: " + $label + " — " + $detail)
    }
  }'
  exit 0
}
