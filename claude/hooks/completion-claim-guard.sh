...
if [[ -z "$transcript_path" || ! -f "$transcript_path" ]] && [[ -d "${HOME}/.claude/projects" ]]; then
  transcript_path=$(find "${HOME}/.claude/projects" -type f -name '*.jsonl' -mmin -10 -print 2>/dev/null \
    | sort \
    | tail -n 1 || true)
fi
