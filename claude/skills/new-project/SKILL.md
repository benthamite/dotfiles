...
- **Failure alerts**: post to `#automation-alerts` (`C0AUU186CGL`) with `<@U0AKT7H6G2H>` mention so Pablo gets notified, via `op://Automations/Slack - Epoch Bot/credential`. Always include a link to the failed GH Actions run. The bot must be a member of the alert channel; have it self-join via `conversations.join` since the app lacks `channels:write.invites`.
