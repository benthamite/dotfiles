...
:END:
Note that we use a simple modification to map ~return_or-enter~ to ~quote~. We can't use a complex modification because, unlike simple modifications, they don't affect how simlayers behave, and we later define a ~quote~ simlayer.
