...
Unfamiliar code execution uses the shared =bin/untrusted-run= VM boundary;
reviewed local workflows retain host execution. The paired routing guard is
registered in =hooks.json= for recognizable untrusted launchers and the
designated staging directory. Its exact definition needs native hook trust.
See [[file:hooks/README.org][the hook contracts]] for coverage and [[file:../bin/README.org][the runner contract]] for prerequisites.

The native =agents.max_concurrent_threads_per_session= setting allows 32
concurrent case workers, excluding the coordinator. This is an explicit
operating limit, not a claimed Codex product maximum. Existing loaded threads
may retain their previous limit; verify activation with actual worker launches.
...
authorized scoped contribution grant for pushes and mutating =gh= operations. It
reads the allowlist and the automations registry from the committed =HEAD=
revision, so a working-tree edit to either confers nothing. It classifies each literal shell command nested in
=functions.exec= independently and refuses dynamic or ambiguous nested calls;
read-only GitHub inspection remains available for undeclared repositories and
may be piped or chained, while a write inside a compound command is refused
before any repository check, including a real push chained after a
=--dry-run= one. A
...
Secret-file exceptions apply per destination in native multi-file patches.
Ordinary-file sections remain scanned even when the patch also changes or
deletes an exempt file. Moves from exempt to ordinary destinations deny because
unchanged source contents are unavailable without reading the file. Native
=Write=/=Edit= exceptions retain their single-destination scope.
...
arbitrary interpreters and mutations remain blocked. Literal nested shell calls
must each pass the same classification. Denials identify the pending package,
commit and repository; inspection never supplies verification evidence.
