...
check_sensitive_read() {
  ...
  SENSITIVE_LABEL=$(sr_label_for_text "$(mask_heredoc_bodies "$COMMAND")")
  [ -z "$SENSITIVE_LABEL" ] && return 0
  if [ -z "$(sr_label_for_text "$(mask_heredoc_bodies "$COMMAND" all)")" ]; then
    add_deny "BLOCKED: sensitive read: ${SENSITIVE_LABEL} — a program fed to an interpreter through a heredoc names this file and the guard cannot prove the program does not read it; write the program to a file and run that, or drop the mention."
    return 0
  fi
...
# --- quoted-content masking for the destructive checks -----------------------
# The destructive checks match shell *syntax*, so string literals must not
# trigger them: a commit message mentioning a force push is not a force push.
# dc_mask_quoted blanks the contents of single-quoted, $'...', and
# $-and-backtick-free double-quoted strings, plus quoted-delimiter heredoc
# bodies; expansions, unquoted syntax, and unquoted-delimiter heredoc bodies
# (which expand) stay verbatim.
#
...
#   heredoc bodies ARE executed, so those commands keep strict raw matching.
...
# Source of truth: claude/hooks/block-destructive-command.sh (mask_quoted);
# keep this inlined copy in sync by hand.
dc_mask_quoted() {
