...
if echo "$COMMAND" | grep -qE '(^|[[:space:];|&])gh[[:space:]]+pr[[:space:]]+(close|reopen|merge|review|ready|lock|unlock|update-branch)\b'; then
...
if echo "$COMMAND" | grep -qE '(^|[[:space:];|&])gh[[:space:]]+issue[[:space:]]+(close|reopen|edit|lock|unlock|transfer|delete|pin|unpin|develop)\b'; then
