...
This is program plan 9 of 12. Start only after the Epoch document-link plan has
cleared every targeted `.gdoc` error while preserving its recorded comparison
set. Re-run the complete symlink audit before either bridge mutation and stop
if any directory or unexpected file symlink exists. Snapshot the live native
error records by exact path and category. Execute in the real Drive-side
working trees, not an isolated worktree, because the acceptance signal is tied
to these exact filesystem paths.
...
- Create: `~/.local/state/drive-workspace-migration/claude-instruction-bridges/<timestamp>/` — mode `0700` category manifest plus two path journals.
...
~~~python
    ...
    def test_import_bridge_with_missing_target_keeps_reminder(self):
        repo = self.make_repo(["CLAUDE.md"])
        self.write_file(repo, "CLAUDE.md", "@AGENTS.md\n")
        self.assertEqual(
            [
                "Project-local CLAUDE.md changed; update AGENTS.md "
                "in the same repo."
            ],
            self.module.reminder_messages([str(repo / "CLAUDE.md")], repo),
        )

    def test_import_bridge_rejects_symlinked_agents_in_python_and_shell(self):
        repo = self.make_instruction_bridge_repo()
        canonical = repo / "CANONICAL.md"
        self.write_file(repo, "CANONICAL.md", "canonical instructions\n")
        (repo / "AGENTS.md").unlink()
        (repo / "AGENTS.md").symlink_to(canonical.name)
        self.assertFalse(
            self.module.is_project_agents_import_bridge(repo / "CLAUDE.md")
        )
        self.assertEqual(
            [
                "Project-local instruction drift after tool-specific "
        ...
        for flags in ((), ("--check",), ("--reverse",)):
            with self.subTest(flags=flags):
                ...
                self.assertEqual("canonical instructions\n", canonical.read_text())
...
def is_project_agents_import_bridge(path: Path) -> bool:
    return (
        ...
        and (path.parent / "AGENTS.md").is_file()
        and not (path.parent / "AGENTS.md").is_symlink()
    )
~~~
...
After the existing check that both files exist, reject an `AGENTS.md` symlink
before either the bridge or ordinary-copy branch can run:

~~~bash
if [ -L "$AGENTS_FILE" ]; then
  echo "mirror-claude-agents: AGENTS.md must be a regular non-symlink file: $AGENTS_FILE" >&2
  exit 1
fi
~~~

Then, before the ordinary `cmp` branch, add:
...
Capture `rubric-visualizer/CLAUDE.md` with `drive-workspace capture-path
--final-type file`, then run `record-path-cloud`. Record the path-journal
reference, `AGENTS.md` hash, Git mode/status, native baseline, and either the
exact pre-existing cloud object ID/parent or the authoritative state
`preexisting_cloud_object: absent` when the exact-parent lookup finds none.
Do not invent an ID for a Drive-rejected symlink; more than one candidate is a
blocker. No mutation may precede this record.

- [ ] **Step 2: Pause, confirm the journal, and replace only the symlink**

Pause Drive and run `drive-workspace confirm-path-paused --journal JOURNAL` for
the Rubric journal. While Drive remains paused, re-read the path and category
journals and require the exact `CLAUDE.md` link text, `AGENTS.md` hash, Git
mode/status, and cloud-preimage state to match Step 1. Move `CLAUDE.md` to
Trash, then create a regular `CLAUDE.md` through `apply_patch` with exactly:
...
While still paused, require the new file to be a mode-`0644` regular file
with exact bytes and require the `AGENTS.md` hash to remain unchanged. On any
failure, run `rollback-path-local` while paused, verify the original symlink and
Git state, resume, and stop. On success, resume Drive before the cloud and
native checks.
...
- [ ] **Step 5: Keep the bridge uncommitted through path-level closure**
...
Expected: only the intended unstaged `CLAUDE.md` representation change. Do not
stage or commit it until Step 6 closes the local, cloud, and native transaction.

- [ ] **Step 6: Restart Drive and require the Rubric bridge error to clear**

Record the restart timestamp, gracefully quit and relaunch Google Drive, wait
for settling, and inspect the native menu-bar error panel. Require the prior
`rubric-visualizer/CLAUDE.md` record to clear and no new record for the regular
bridge. Require every other baseline record to remain unchanged. Stop the plan
if the path-level result or error class differs.
Run `verify-path-local` and `verify-path-cloud`, append the native transition,
and fsync the category manifest. On failure, pause Drive and obtain a fresh
successful `confirm-path-paused` record. If the journal has pre-existing object
IDs, run `restore-path-cloud` for only those IDs; if it records
`preexisting_cloud_object: absent`, do not call cloud restore and instead
require absence at the exact parent. Run `rollback-path-local` while paused,
resume, and require the exact symlink, authoritative cloud-preimage state,
clean baseline Git state, and baseline native record before stopping.
...
Require one local commit, no push, and a final `verify-path-local` result that
records the committed mode-`100644` representation.

### Task 6: Fast-forward Uqbar without touching secrets
...
git rev-list --left-right --count HEAD...origin/main
git diff --name-only HEAD..origin/main -- secrets/KNOWN_HOSTS.txt secrets/SSH_KEY.txt
~~~

Require:

- no tracked or staged dirty paths;
- only `secrets/KNOWN_HOSTS.txt` and `secrets/SSH_KEY.txt` untracked;
- `HEAD` is an ancestor of `origin/main`;
- neither untracked path appears in the upstream diff.

Stop if any condition changes. Do not stash, reset, rebase, or delete either secret file.
...
Run `shasum -a 256 AGENTS.md`. Before mutation, capture `uqbar/CLAUDE.md` with
`drive-workspace capture-path --final-type file`, run `record-path-cloud`, and
append the path journal, Git state, canonical hash, native baseline, and either
the exact pre-existing cloud object ID/parent or
`preexisting_cloud_object: absent` to the category manifest. More than one
exact-parent candidate is a blocker. Pause Drive, run `drive-workspace
confirm-path-paused --journal JOURNAL`, and while paused recheck the exact link
text, canonical hash, Git state, and authoritative cloud-preimage state. Move
only `CLAUDE.md` to Trash, then create a regular `CLAUDE.md` through
`apply_patch` with exactly:

~~~markdown
@AGENTS.md
~~~

While still paused, require exact bytes, mode `0644`, and an unchanged
`AGENTS.md` hash. On failure, run `rollback-path-local` while paused, verify the
exact preimage and Git state, resume, and stop. On success, resume Drive before
the project-memory, cloud, or native checks.
...
- [ ] **Step 3: Keep `CLAUDE.md` uncommitted through path-level closure**
...
Confirm the two untracked secret files remain present and unstaged. Do not
stage or commit `CLAUDE.md` before Step 4 closes its local, cloud, and native
transaction.

- [ ] **Step 4: Restart Drive and require the Uqbar bridge error to clear**

After settling, require the prior `uqbar/CLAUDE.md` record to clear and no new
record for the regular bridge. Require every other baseline record to remain
unchanged. The next plan starts only after both bridge paths pass this
path-level gate.
Run both path verification commands, append and fsync the second transition,
and require the category manifest to contain two complete transactions. On
failure, pause Drive and obtain a fresh successful `confirm-path-paused` record.
If the journal has pre-existing object IDs, restore only those exact IDs; if it
records `preexisting_cloud_object: absent`, skip cloud restore and require
absence at the exact parent. Run `rollback-path-local` while paused, verify the
exact symlink and clean baseline Git state, resume, and stop.
...
Confirm the two untracked secret files remain present and unstaged, require one
local commit and no push, and append a final `verify-path-local` result for the
committed mode-`100644` bridge.
