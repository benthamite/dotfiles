...
| =spacebar=      | =⌘Tab= (switch apps)            | =C-= | Control        |
...
|-----------------+-----------------------------------+---------|
| =left_control=  | =Escape=                    | =⌥=  |
| =left_option=   | =Enter= (toggle tabs)       | =⇧=  |
| =left_command=  | Switch to last tab (=⌘⌃0=) | =⌘=  |
| =spacebar=      | =⌘Tab= (switch apps)        | =⌃=  |
| =right_command= | Spacebar                    | ---   |
| =right_option=  | =Tab=                       | =⌥=  |

The =spacebar= key also has a combination effect outside Emacs: pressing =left_command= + =spacebar= triggers =⌘`= (move focus to next window).
...
#+begin_src clojure
{:des "Outside Emacs: left_command → other tab (alone) | ⌘ (held)"
 :rules [
