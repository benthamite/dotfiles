...
contains_op_reveal_output() {
  local command="$1"
  { printf '%s' "$command" | grep -qE '(^[[:space:]]*|[;&|(!][[:space:]]*)(op-automations|((/usr/bin/|/bin/)?env)[[:space:]]+-u[[:space:]]+OP_SERVICE_ACCOUNT_TOKEN[[:space:]]+(/opt/homebrew/bin/|/usr/local/bin/|/usr/bin/)?op|(/opt/homebrew/bin/|/usr/local/bin/|/usr/bin/)?op)[[:space:]]+' || \
    { is_explicit_desktop_op_batch "$command" && printf '%s' "$command" | grep -qE '(^|[;&|[:space:]])op[[:space:]]+'; }; } && \
