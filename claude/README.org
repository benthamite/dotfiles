...
The secret-leak guard denies direct agent-shell invocations of raw =op=, the
...
use a dedicated audited operation that keeps values out of agent output. The
Claude Bash dispatcher delegates this decision to the standalone guard instead
of carrying a second inline policy copy. Quoted argument globs, including =rg=
