...
- After cloning, it initializes submodules (`git submodule init`, `git submodule update --recursive`).
...
+ ~vc-extras--initialize-submodules~: Runs `git submodule init` and `update --recursive`.
