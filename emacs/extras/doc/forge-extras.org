...
- Programmatic calls can pass an optional list of items as the third argument. If provided, this list is used directly, bypassing the GitHub API call.
- It does not filter by repository, listing items from all repositories present in the project (when fetching).
- The items are listed in the order they appear on the project board (when fetching).
- The command returns the list of fetched or provided items as Elisp plists.
- The returned list is always cached in the internal variable ~forge-extras--cached-project-items~.
