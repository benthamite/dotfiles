...
(defcustom claude-code-extras-rename-model "haiku"
  "Model to use for generating session names.
Naming is a lightweight task that does not require the most capable
model.  Supports aliases like \"haiku\", \"sonnet\", \"opus\" as
well as full model IDs."
  :type '(choice (const :tag "Haiku (latest)" "haiku")
                 (const :tag "Sonnet (latest)" "sonnet")
                 (const :tag "Opus (latest)" "opus")
                 (const :tag "Use batch default" nil)
                 string)
  :group 'claude-code-extras)

(defun claude-code-extras--session-projects-dir (&optional account)
  "Return the projects directory for ACCOUNT.
When ACCOUNT is nil, use the current account."
  (let* ((account-name (or account (claude-code-extras--resolve-account)))
         (config-dir (if account-name
                        (expand-file-name
                         (alist-get account-name
                                    claude-code-extras-accounts
                                    nil nil #'string=))
                      (expand-file-name "~/.claude"))))
    (expand-file-name "projects" config-dir)))

(defun claude-code-extras--session-project-dirs ()
  "Return a list of project directory names under the sessions projects dir."
  (let ((dir (claude-code-extras--session-projects-dir)))
    (when (file-directory-p dir)
      (directory-files dir nil "\\`[^.]"))))

(defun claude-code-extras--session-jsonl-files (project-dir-name)
  "Return JSONL files for PROJECT-DIR-NAME.
PROJECT-DIR-NAME is the encoded directory name under the projects dir."
  (let ((dir (expand-file-name project-dir-name
                               (claude-code-extras--session-projects-dir))))
    (when (file-directory-p dir)
      (directory-files dir t "\\.jsonl\\'"))))

(defun claude-code-extras--session-parse-metadata (jsonl-file)
  "Parse session metadata from JSONL-FILE.
Returns a plist with :session-id, :first-prompt, :custom-title,
:timestamp, and :file."
  (let (session-id first-prompt custom-title timestamp)
    (with-temp-buffer
            ...
            (condition-case nil
                (let ((obj (json-parse-string line :object-type 'plist)))
                  (pcase (plist-get obj :type)
                    ("user"
                     (unless first-prompt
                       (when-let* ((msg (plist-get obj :message))
                                   (content (plist-get msg :content)))
                         (when (stringp content)
                           (setq first-prompt content))))
                     (unless session-id
                       (setq session-id (plist-get obj :sessionId)))
                     (unless timestamp
                       (setq timestamp (plist-get obj :timestamp))))
                    ("custom-title"
                     (setq custom-title (plist-get obj :customTitle)))))
              (error nil))))
        (forward-line 1)))
    (when session-id
...
(defun claude-code-extras--session-extract-context (jsonl-file)
  "Extract conversation context from JSONL-FILE for name generation.
Returns the first few user prompts concatenated, truncated to 1000 chars."
  (let ((prompts '())
        (total 0))
    (with-temp-buffer
      (insert-file-contents jsonl-file)
      (goto-char (point-min))
      (while (and (not (eobp)) (< total 1000))
        (let ((line (buffer-substring-no-properties
                     (line-beginning-position) (line-end-position))))
          (unless (string-empty-p line)
            (condition-case nil
                (let ((obj (json-parse-string line :object-type 'plist)))
                  (when (equal (plist-get obj :type) "user")
                    (when-let* ((msg (plist-get obj :message))
                                (content (plist-get msg :content)))
                      (when (stringp content)
                        (push content prompts)
                        (setq total (+ total (length content)))))))
              (error nil))))
        (forward-line 1)))
    (let ((text (string-join (nreverse prompts) "\n")))
      (if (> (length text) 1000)
          (substring text 0 1000)
        text))))

(defun claude-code-extras--session-append-custom-title (jsonl-file session-id title)
  ...
  (let ((entry (json-serialize
    ...
    (with-temp-buffer
      (insert entry "\n")
      (append-to-file (point-min) (point-max) jsonl-file))))

(defun claude-code-extras--session-collect-unnamed (project-dir-name)
  "Collect unnamed sessions from PROJECT-DIR-NAME.
Returns a list of metadata plists for sessions lacking a customTitle."
  (let ((files (claude-code-extras--session-jsonl-files project-dir-name))
        (unnamed '()))
    (dolist (file files)
      (when-let* ((meta (claude-code-extras--session-parse-metadata file)))
        (unless (plist-get meta :custom-title)
          (push meta unnamed))))
    (nreverse unnamed)))
...
(defun claude-code-extras-rename-sessions ()
  "Generate names for all unnamed sessions in a project.
Prompts for a project, then uses Claude to generate a short
kebab-case name for each unnamed session based on its conversation
context.  Names are written as custom-title entries in the session
JSONL files, making them visible in the /resume picker."
  (interactive)
  (let* ((projects (claude-code-extras--session-project-dirs))
         ...
         (display (completing-read
                   "Project: "
                   (mapcar #'car annotated) nil t))
         (project (cdr (assoc display annotated)))
         (unnamed (claude-code-extras--session-collect-unnamed project)))
    (if (null unnamed)
        (message "All sessions in %s already have names" display)
      (when (yes-or-no-p
             (format "Generate names for %d unnamed session(s)?" (length unnamed)))
        (claude-code-extras--rename-sessions-batch unnamed)))))

(defun claude-code-extras--rename-sessions-batch (sessions)
  "Generate and apply names for SESSIONS in batches.
SESSIONS is a list of metadata plists from
`claude-code-extras--session-collect-unnamed'.  Sessions are
processed in batches of up to 20, sending all first prompts to
Claude in a single request for efficiency."
  (let ((state (list :queue sessions
                     :results nil
                     :start-time (current-time)
                     :batch-index 0)))
    (message "Renaming %d session(s)..." (length sessions))
    (claude-code-extras--rename-next-batch state)))

(defun claude-code-extras--rename-next-batch (state)
  "Process the next batch of sessions in STATE."
  (let ((queue (plist-get state :queue)))
    (if (null queue)
        (claude-code-extras--rename-finish state)
      (let* ((batch (seq-take queue 20))
             (rest (seq-drop queue 20))
             (batch-num (1+ (plist-get state :batch-index)))
             (total-batches (1+ (/ (max 0 (1- (+ (length batch) (length rest))))
                                   20)))
             (prompt (claude-code-extras--rename-build-batch-prompt batch)))
        (plist-put state :queue rest)
        (plist-put state :batch-index batch-num)
        (message "Naming batch %d/%d (%d sessions)..."
                 batch-num total-batches (length batch))
        (claude-code-extras--run-prompt
         prompt
         :model (or claude-code-extras-rename-model "haiku")
         :max-turns 1
         :callback
         (lambda (result)
           (let ((names (when (zerop (plist-get result :exit-code))
                          (claude-code-extras--rename-parse-batch-response
                           (or (plist-get result :text) "")))))
             (dolist (session batch)
               (let* ((sid (plist-get session :session-id))
                      (name (cdr (assoc sid names)))
                      (valid (and name
                                  (not (string-empty-p name))
                                  (string-match-p "\\`[a-z0-9-]+\\'" name))))
                 (if valid
                     (progn
                       (claude-code-extras--session-append-custom-title
                        (plist-get session :file) sid name)
                       (push (list :session-id sid :name name :status "renamed")
                             (plist-get state :results)))
                   (push (list :session-id sid
                               :name (or name "(no match)")
                               :status "failed")
                         (plist-get state :results))))))
           (claude-code-extras--rename-next-batch state)))))))

(defun claude-code-extras--rename-build-batch-prompt (sessions)
  "Build a prompt asking Claude to name all SESSIONS at once.
Returns a string with the system instruction and session contexts."
  (let ((parts '()))
    (push (concat
           "Generate a short kebab-case name (2-4 words) for each session below. "
           "Use lowercase words separated by hyphens. "
           "Examples: \"fix-login-bug\", \"add-auth-feature\", "
           "\"refactor-api-client\", \"debug-test-failures\".\n\n"
           "Reply with EXACTLY one line per session in the format:\n"
           "SESSION_ID: name\n\n"
           "No other text, no explanations.")
          parts)
    (dolist (session sessions)
      (let* ((sid (plist-get session :session-id))
             (context (claude-code-extras--session-extract-context
                       (plist-get session :file)))
             (truncated (if (> (length context) 200)
                            (substring context 0 200)
                          context)))
        (push (format "--- %s ---\n%s" sid truncated) parts)))
    (string-join (nreverse parts) "\n\n")))

(defun claude-code-extras--rename-parse-batch-response (text)
  "Parse TEXT as batch rename response.
Returns an alist of (SESSION-ID . NAME) pairs."
  (let ((result '()))
...
(defun claude-code-extras--rename-finish (state)
  "Display results for the rename batch in STATE."
  (let* ((results (nreverse (plist-get state :results)))
         (total (length results))
         (renamed (cl-count "renamed" results
                            :key (lambda (r) (plist-get r :status))
                            :test #'string=))
         (elapsed (float-time
                   (time-subtract (current-time)
                                  (plist-get state :start-time)))))
    (message "Renamed %d/%d session(s) in %.1fs" renamed total elapsed)
    (when results
      (let ((buf (get-buffer-create "*Claude Session Rename*")))
        (with-current-buffer buf
          (let ((inhibit-read-only t))
            (erase-buffer)
            (insert (format "Session rename results (%d/%d, %.1fs)\n\n"
                            renamed total elapsed))
            (dolist (r results)
              (insert (format "  %-10s %-30s %s\n"
                              (plist-get r :status)
                              (or (plist-get r :name) "-")
                              (plist-get r :session-id)))))
          (special-mode)
          (goto-char (point-min)))
        (display-buffer buf)))))
