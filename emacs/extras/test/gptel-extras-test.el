...
(ert-deftest gptel-extras-test-bib-processing-invalid-key-blocks-without-ui ()
  "Invalid keys block unattended processing without invoking manual generation."
  (require 'ebib-extras)
  (let* ((db (ebib-db-new-database))
         (ebib--databases (list db))
         (ebib--cur-db db)
    ...
    (cl-letf (((symbol-function 'ebib--get-key-at-point) (lambda () key))
              ((symbol-function 'ebib-generate-autokey)
               (lambda () (ert-fail "Interactive key generation")))
              ((symbol-function 'y-or-n-p)
               (lambda (&rest _) (ert-fail "Interactive prompt")))
      ...
      (let ((result (gptel-extras--process-bib-entry-headless 0)))
        (should (equal (plist-get result :key) "a1b"))
        (should (eq (plist-get result :status) 'blocked))
        (should (= (plist-get result :pending) 0))
        (should-not attached)
        (should-not saved)
        (should-not (plist-get result :files))))))

(ert-deftest gptel-extras-test-bib-processing-ignores-stale-cached-key ()
  ...
  (let* ((db (ebib-db-new-database))
         ...
         (ebib-extras--annas-archive-pending-key nil)
         attached waited)
    (ebib-db-set-filename "/tmp/stale-key.bib" db)
    (ebib-db-set-entry key '(("title" . "Paper")) db)
    (ebib-db-set-entry other '(("title" . "Film")) db)
    (ebib-db-set-current-entry-key other db)
    (cl-letf (((symbol-function 'ebib--get-key-at-point) (lambda () key))
              ((symbol-function 'ebib-extras-get-or-set-language) #'ignore)
              ((symbol-function 'ebib-extras-attach-files)
              ...
              ((symbol-function 'gptel-extras--wait-for-bib-attachments)
               (lambda (actual _timeout actual-db &rest _)
                 ...
                 '("/tmp/intended.pdf")))
              ((symbol-function 'ebib-extras-get-field) (lambda (&rest _) nil)))
      (let ((result (gptel-extras--process-bib-entry-headless 0 key db)))
        (should (equal (plist-get result :key) key))
        (should (equal (plist-get result :files) '("/tmp/intended.pdf")))
        (should (equal attached key))
  ...
  (dolist (switch-database '(nil t))
    (let* ((db (ebib-db-new-database))
         (ebib--databases (list db))
           ...
           (point-key target)
           crossref waited saved)
      (ebib-db-set-filename "/tmp/owned.bib" db)
      (ebib-db-set-entry target '(("title" . "Paper")) db)
      (ebib-db-set-entry "Other2020Paper" '(("title" . "Other")) db)
      (ebib-db-set-entry target '(("title" . "Other database")) other-db)
...
(ert-deftest gptel-extras-test-bib-language-rechecks-selection-before-writing ()
  ...
  (let* ((db (ebib-db-new-database))
    ...
    (with-temp-buffer
      ...
      (cl-letf (((symbol-function 'ebib--get-key-at-point) (lambda () key))
                ((symbol-function 'ebib-extras-get-field) (lambda (&rest _) nil))
                ((symbol-function 'tlon-lookup) (lambda (&rest _) nil))
                ((symbol-function 'tlon-lookup-all) (lambda (&rest _) '("english")))
                ((symbol-function 'completing-read)
                 (lambda (&rest _) (setq key "Other2020Paper") "english"))
                ((symbol-function 'ebib-extras-set-field)
                 (lambda (&rest _) (setq written t))))
        (should-error (ebib-extras-get-or-set-language "Author2020Paper" db)
                      :type 'user-error)
        (should-not written)))))

(ert-deftest gptel-extras-test-bib-language-explicit-target-outside-ebib ()
  "An explicit target wins over an unrelated BibTeX or PDF buffer."
  (require 'ebib-extras)
      ...
      (with-temp-buffer
        ...
        (cl-letf (((symbol-function 'ebib--get-key-at-point) (lambda () key))
                  ((symbol-function 'ebib-extras-get-field)
                   (lambda (_field actual) (should (equal actual key)) nil))
                  ((symbol-function 'bibtex-extras-get-field)
                   (lambda (&rest _) (ert-fail "Read unrelated BibTeX buffer")))
                  ((symbol-function 'bibtex-set-field)
                   (lambda (&rest _) (ert-fail "Wrote unrelated BibTeX buffer")))
                  ((symbol-function 'tlon-lookup) (lambda (&rest _) nil))
                  ((symbol-function 'tlon-lookup-all) (lambda (&rest _) '("english")))
                  ((symbol-function 'completing-read) (lambda (&rest _) "english"))
                  ((symbol-function 'ebib-extras-set-field)
                   (lambda (field value) (setq written (list field value)))))
          (ebib-extras-get-or-set-language key db)
          (should (equal written '("langid" "english"))))))))

(ert-deftest gptel-extras-test-bib-processing-allows-pdf-buffer-change ()
...
(ert-deftest gptel-extras-test-bib-import-does-not-use-ambient-browser-buffer ()
  "Explicit imports neither copy browser URLs nor disturb the caller's buffer."
  (require 'zotra-extras)
  (dolist (mode '(eww-mode elfeed-show-mode))
    (with-temp-buffer
      (setq major-mode mode)
      (insert "Caller text")
      (let ((caller (current-buffer)) (original-point (point))
            (kill-ring '("Unchanged")) (interprogram-cut-function nil))
        (cl-letf (((symbol-function 'eww-copy-page-url)
                   (lambda () (ert-fail "Copied ambient browser URL")))
                  ((symbol-function 'elfeed-extras-kill-link-url-of-entry)
                   (lambda () (ert-fail "Copied ambient feed URL")))
                  ((symbol-function 'zotra-extras--check-target-buffer) #'ignore)
                  ((symbol-function 'zotra-extras--add-and-maybe-open)
                   (lambda (identifier _format file do-not-open)
                     (should (equal identifier "10.123/example"))
                     (should (equal file "fixture.bib"))
                     (should do-not-open)
                     "Imported2026Entry")))
          (should (equal (gptel-extras-add-bib-entry "10.123/example" "fixture.bib")
                         "Imported2026Entry")))
        (should (eq (current-buffer) caller))
        (should (= (point) original-point))
        (should (equal kill-ring '("Unchanged")))))))

(defmacro gptel-extras-test-with-background-bib (&rest body)
  "Run BODY with a real bibliography file and isolated Ebib database state."
  (declare (indent 0))
  `(let* ((file (make-temp-file "gptel-background-" nil ".bib"))
          (other (ebib-db-new-database))
          (ebib--databases (list other))
          (ebib--cur-db other)
          (log-buffer (generate-new-buffer " *gptel-background-log*"))
          (ebib--buffer-alist (list (cons 'log log-buffer)))
          (key "Author2026Paper"))
     (unwind-protect
         (progn
           (ebib-db-set-filename "/tmp/unrelated-background.bib" other)
           (ebib-db-set-entry key (copy-tree '(("=type=" . "article") ("title" . "{Other}"))) other)
           (ebib-db-set-current-entry-key key other)
           (with-temp-file file (insert "@article{Author2026Paper, title={Original}}\n"))
           ,@body)
       (kill-buffer log-buffer)
       (delete-file file))))

(ert-deftest gptel-extras-test-background-bib-load-and-refresh-preserve-ui ()
  "Load and refresh the requested database without selecting or displaying it."
  (require 'ebib-extras)
  (gptel-extras-test-with-background-bib
    (with-temp-buffer
      (insert "Caller text")
      (let ((caller (current-buffer)) (original-point (point))
            (windows (window-state-get nil t)) db)
        (cl-letf (((symbol-function 'ebib) (lambda (&rest _) (ert-fail "Ebib UI")))
                  ((symbol-function 'switch-to-buffer) (lambda (&rest _) (ert-fail "Buffer switch")))
                  ((symbol-function 'display-buffer) (lambda (&rest _) (ert-fail "Buffer display")))
                  ((symbol-function 'ebib-extras-sort) (lambda (&rest _) (ert-fail "UI sorting"))))
          (setq db (gptel-extras--open-bib-entry-for-processing file key))
          (should-not (eq db other))
          (should (equal (ebib-db-get-field-value "title" key db) "{Original}"))
          (let ((entry (ebib-db-get-entry key db)))
            (should (eq db (gptel-extras--open-bib-entry-for-processing file key)))
            (should (eq entry (ebib-db-get-entry key db))))
          (with-temp-file file (insert "@article{Author2026Paper, title={Refreshed}}\n"))
          (ebib-db-set-modtime '(0 0) db)
          (should (eq db (gptel-extras--open-bib-entry-for-processing file key)))
          (should (equal (ebib-db-get-field-value "title" key db) "{Refreshed}")))
        (should (eq ebib--cur-db other))
        (should (equal (ebib--db-get-current-entry-key other) key))
        (should (equal (ebib-db-get-field-value "title" key other) "{Other}"))
        (should (eq (current-buffer) caller))
        (should (= (point) original-point))
        (should (equal (window-state-get nil t) windows))))))

(ert-deftest gptel-extras-test-background-bib-refuses-dirty-state ()
  "A refresh refuses unsaved database and visiting-buffer edits before mutation."
  (require 'ebib-extras)
  (gptel-extras-test-with-background-bib
    (let* ((db (gptel-extras--open-bib-entry-for-processing file key))
           (entry (ebib-db-get-entry key db)))
      (ebib-db-set-modified t db)
      (should-error (gptel-extras--open-bib-entry-for-processing file key) :type 'user-error)
      (should (eq entry (ebib-db-get-entry key db)))
      (ebib-db-set-modified nil db)
      (with-temp-buffer
        (setq buffer-file-name file)
    ...
    (unwind-protect
        (cl-letf (((symbol-function 'require)
                  ...
                  ((symbol-function 'gptel-extras--process-bib-entry-headless)
                   (lambda (timeout key db)
                     (push (list 'process timeout key db) calls)
          ...
          (should
           (equal (nreverse calls)
                  '((add "10.123/example" nil "new.bib" t)
                    ...
                    (process 7 "smith2024test" target-db)))))
      (if old-key-bound
