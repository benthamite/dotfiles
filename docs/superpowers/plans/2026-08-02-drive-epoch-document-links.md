...
This is program plan 8 of 12. Start only after the residual-directory repair
plan has produced a fresh audit with zero directory symlinks under `~/My
Drive` and exactly the 20 journaled regular-file links owned by plans 9 through
11, the native stale-record convergence pilot has passed, and Drive has
completed a clean restart. Snapshot the live panel by exact path and category before each
mutation; stop and re-triage if an intended source path is absent or any new
category appears. Execute in the real dotfiles and Epoch working trees, not
isolated worktrees, because the generated `.url` files and native Drive records
must be verified at their exact live paths. Before Google API checks, read
`claude/context/google-services.md` and `claude/context/secrets.md`; never print
OAuth tokens.

Before execution, set and export `EPOCH_ACCOUNT_EMAIL` and
`PERSONAL_ACCOUNT_EMAIL` from the owning account configuration. Meeting paths
below are synthetic examples; substitute a privately selected pilot path
before execution and stop if example values remain.
...
- Create outside Drive: `~/.local/state/drive-workspace-migration/epoch-document-links/<timestamp>/` — mode `0700` category journal, private source preimages, permission hashes, path-transaction references, and native/cloud evidence.
...
Preserve the twelve personal-account `.gdoc` files and all unrelated dirty state.
...
class EpochDocLinkTests(unittest.TestCase):
    ...
    def test_create_is_atomic_idempotent_and_mode_0644(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.module.create_shortcut("First_ID", destination)
            self.assertEqual(
                self.module.shortcut_text("First_ID"),
                destination.read_text(),
            )
            self.assertEqual(0o644, stat.S_IMODE(destination.stat().st_mode))
            self.assertEqual([], list(root.glob(".meeting.url.*")))

    def test_create_refuses_different_existing_regular_content(self):
        with tempfile.TemporaryDirectory() as temporary:
            destination = Path(temporary) / "meeting.url"
...
def create_shortcut(document_id: str, destination: Path) -> None:
    ...
    content = shortcut_text(document_id)
    if destination.exists():
        ...
        if existing == content:
            if stat.S_IMODE(destination.stat().st_mode) != 0o644:
                raise LinkError(
                    f"Existing shortcut has unexpected mode: {destination}"
                )
            return
        raise LinkError(
            f"Refusing to replace different regular file: {destination}"
        )
...
Expected bundle ID: `com.stafforini.epoch-doc-handler`. Require the configured Epoch alias row to match the owning account's private browser configuration; do not copy the profile directory or account identity into this public plan.
...
Create a mode-`0700` temporary directory, leave a child path named
`missing-config.json` absent, set `CHROME_PROFILE_OPEN_CONFIG` to that absent
child, and run a syntactically valid link through the CLI. Require nonzero
status, require the child to remain absent, and require no
default/personal-profile tab. Move the temporary directory to Trash. Finally
open a malformed `epoch-doc:` URL through LaunchServices and confirm the native
app displays its critical error alert.
...
- [ ] **Step 1: Reconfirm the path-level native baseline**

Restart Drive, wait for settling, and export the native error inventory. Require
all 36 intended Epoch-account `.gdoc` paths to be present as
`INVALID_GOOGLE_DOCUMENT`, with no error attached to any of the twelve personal
account pointers. Record every other error path and category as the comparison
set for this plan. If the intended set differs, re-triage rather than inferring
success or failure from a total.
...
- [ ] **Step 3: Capture all 36 rollback preimages and all 48 pointer hashes**

Create the external category directory with mode `0700` and a fsynced
mode-`0600` `manifest.jsonl`. For each of the 36 pointers whose `.email` is
`$EPOCH_ACCOUNT_EMAIL`, run `drive-workspace capture-path` with the `.gdoc` as
`--path`, its sibling `.url` as `--replacement`, and `--final-type absent`,
then run `record-path-cloud` for the returned journal. Append the transaction
directory, relative source path, exact case-preserved document ID, source
SHA-256 and mode, SHA-256 of normalized Epoch permission JSON, and one of these
two authoritative cloud-preimage states:

- the exact pre-existing object ID and parent when the exact-parent lookup
  finds one object; or
- `preexisting_cloud_object: absent` when the exact-parent lookup finds none.

Do not synthesize an ID from the document ID, and do not treat an absent
preimage as missing evidence. More than one candidate remains an ambiguity and
stops the plan. Retain the private source bytes in the transaction directory.

For the twelve preserved personal pointers, append path, mode, source SHA-256,
parsed account, and exact cloud ID/parent. Fsync and re-read every journal,
preimage, and category record. Require exactly 36 migration records and 12
preserve records before creating a `.url`. Any unreadable pointer,
permission-query failure, duplicate path, or ambiguous cloud object stops the
plan. Never print IDs or permission bodies.

- [ ] **Step 4: Pause, confirm the pilot transaction, and create the pilot**

Read the case-preserved ID from the exact source pointer being replaced; never
substitute another document. Export it without printing it:

~~~bash
EPOCH_PILOT_DOC_ID=$(jq -er .doc_id meetings/example-meeting/YYYY-MM-DD.gdoc)
export EPOCH_PILOT_DOC_ID
~~~

The ID is deliberately not written in this plan because it is a direct handle
to internal notes. Run:
...
Normalize and hash the response and require it to match the pilot permission
hash in the external manifest. Require no explicit permission whose
`emailAddress` is `$PERSONAL_ACCOUNT_EMAIL`. Existing domain or link access
is not treated as a newly added personal permission.

Pause Drive and run `drive-workspace confirm-path-paused --journal JOURNAL`
for the pilot journal. While Drive remains paused, re-read the journal and
require the source bytes, mode, parsed account, case-preserved document ID,
permission hash, and authoritative cloud-preimage state to match Step 3.
Require the sibling `.url` path to remain absent. Only then create:
...
Require mode `0644`, the exact bytes above, and an unchanged `.gdoc` source
while still paused. If creation or immediate local verification fails, require
the source still matches its preimage, move only the journaled `.url`
replacement to Trash, verify it is absent, resume Drive, and stop. Otherwise
resume Drive before the browser, permission, cloud, or native checks below.

- [ ] **Step 5: Open through LaunchServices and verify the real browser**
...
- [ ] **Step 6: Prove permissions did not change**
...
- [ ] **Step 7: Pause again, trash the pilot `.gdoc`, and require its error to clear**

Pause Drive again and obtain a fresh successful `confirm-path-paused` record
for the pilot journal. While paused, require the source preimage and permission
hash still match Step 3 and the `.url` still matches the intended bytes and
mode. Move only `meetings/example-meeting/YYYY-MM-DD.gdoc` to Trash, verify the source is
absent and the replacement remains exact. If either immediate local check
fails, keep Drive paused and enter the Step 8 rollback branch. Otherwise resume
Drive. Restart Drive, wait for settling, and require the pilot's
`INVALID_GOOGLE_DOCUMENT` record to clear, with no error for the new `.url` and
no change to the comparison set recorded in Step 1.

- [ ] **Step 8: Verify the uploaded cloud path and journal the transition**
...
Run `verify-path-local` and `verify-path-cloud` for the pilot transaction,
append the exact native transition and evidence hashes to the category
manifest, and fsync it. If any check from Steps 5 through 8 fails, keep Drive
paused if it is already paused; otherwise pause it. Obtain a fresh successful
`confirm-path-paused` record. If the journal has
pre-existing object IDs, run `restore-path-cloud` for only those exact IDs; if
it records `preexisting_cloud_object: absent`, do not call cloud restore and
instead require the exact parent still has no preimage object or duplicate.
Then run `rollback-path-local`, resume, and require the original bytes, mode,
authoritative cloud-preimage state, permission hash, and native record to match
the Step 1 baseline before stopping the plan.
...
Use `find -print0` plus `jq` to select only JSON pointers whose `.email` equals `$EPOCH_ACCOUNT_EMAIL`. Require 35 remaining after the pilot. Record the twelve personal pointer paths privately before migration and require each recorded path and hash to remain unchanged. Do not copy that inventory into this public plan.

- [ ] **Step 2: Pause, confirm all 35 transactions, and create every replacement**

Pause Drive. Run `drive-workspace confirm-path-paused --journal JOURNAL` for
each of the 35 remaining migration journals and require all confirmations to be
fresh before changing any path. While paused, re-read every source and require
its bytes, mode, parsed account, case-preserved document ID, permission hash,
and authoritative cloud-preimage state to match the category manifest; require
every sibling `.url` to be absent. Then run this from the Epoch root while
Drive remains paused:
...
while IFS= read -r -d "" gdoc_file; do
  ...
  [ "$account" = "$EPOCH_ACCOUNT_EMAIL" ] || continue
...
or ignore-rule failure, before removing any `.gdoc` source. Require all 35
sources to remain exact and all replacements created so far to match their
intended bytes and modes while still paused. On any failure, require every
source still matches its preimage, move only each journaled replacement created
in this batch to Trash, verify all 35 preimages and absent replacements, resume,
and stop. On success, require all 35 replacements, then resume Drive before
opening documents or querying permissions.

- [ ] **Step 3: Verify all 36 real opening paths and permission hashes**
...
For each document, query and normalize permissions again and require its hash
to match the category manifest with no explicit personal email permission.

- [ ] **Step 4: Pause again and move only the 35 remaining Epoch `.gdoc` files to Trash**

After every real opening check passes, pause Drive and obtain a fresh successful
`confirm-path-paused` record for every one of the 35 journals. While paused,
recheck every source preimage and every replacement against the category
manifest. Then run:
...
Do not use a broad `*.gdoc` deletion. Re-run the inventory while still paused
and require:

- zero `.gdoc` files with `.email == "$EPOCH_ACCOUNT_EMAIL"`;
- all twelve privately recorded personal `.gdoc` files still exist;
- exactly 36 `.url` files contain canonical `epoch-doc:` values.

If these local checks fail, keep Drive paused and enter Step 6 rollback. If
they pass, resume Drive before the restart and cloud/native checks.

- [ ] **Step 5: Require all 36 target errors to clear and seal their journals**

Restart Drive and wait for settling. Require no native error for any of the 36
replaced Epoch-account `.gdoc` paths or their `.url` replacements. Require all
twelve personal pointers to remain present and error-free, and require the
comparison set recorded in Task 9 Step 1 to be unchanged. Any new
`INVALID_GOOGLE_DOCUMENT`, `.url`, or unrelated path error stops the plan.

- [ ] **Step 6: Roll back the complete bulk boundary on any failed check**

If any bulk local, browser, permission, cloud, or native check fails, keep Drive
paused if it is already paused; otherwise pause it. Obtain a fresh successful
`confirm-path-paused` record for all 35 journals, and re-confirm every changed
`.url` against its journal. For a source
not yet moved to Trash, require its exact preimage still present and move only
its generated `.url` to Trash. For a trashed source whose journal contains
pre-existing object IDs, run `restore-path-cloud` for only those exact IDs and
then `rollback-path-local`. For a trashed source whose authoritative cloud
preimage is absent, do not call cloud restore: require absence at the exact
parent and run `rollback-path-local`. Never select a Trash item by name.
Require all 35 exact `.gdoc` preimage bytes and modes, their exact recorded IDs
and parents or authoritative absent states, and all permission hashes back;
also require all 35 generated `.url` files absent, the twelve preserved hashes
unchanged, and the native panel restored to the post-pilot baseline. Resume
only after local and cloud-preimage checks pass, then stop the program for
diagnosis.

On success, run `verify-path-local` and `verify-path-cloud` for all 36
transactions, append every native transition and evidence hash, fsync the
category manifest, and require it to reference no `/tmp` evidence.

- [ ] **Step 7: Recheck cloud routing and clean temporary probes**
...
The instruction-bridge plan starts only after this exact path-level gate is met.
