...
3. Add and process the entry through Zotra/Ebib.
   ...
   - Headless/agent workflow: prefer `add_bib_entry_and_process` when available. It imports through Zotra, opens the generated key in Ebib, runs the Ebib processing path noninteractively, waits a bounded time for attachment work, and returns the key plus attached-file status. Pass the resolved target explicitly.
...
4. Complete post-processing.
   ...
   - The bundled DOI downloader is an opt-in fallback, not an automatic response to a fragile Emacs/browser session. Use it only after the normal route's concrete failure has been diagnosed and this fallback is justified and approved. Ensure no other editor/import is writing the target: the helper serializes its own instances and checks for disk changes, but cannot lock other applications or see unsaved buffers. Resolve the script relative to this loaded skill directory, not the current project:
     ```bash
     python3 "SKILL-DIR/scripts/download_annas_article.py" DOI CITEKEY "RESOLVED-BIBFILE"
     ```
     It uses the established Anna's Archive credential, requires an unambiguous matching DOI entry with no existing file association, and refuses unsafe destinations, overwrites, ambiguous lookup results and non-PDF/hash-mismatched downloads. It supports a narrow BibTeX syntax and fails closed on unsupported input. Keep the configured credential origin pinned; do not invent mirror domains after a network error. Hash integrity is not evidence that the PDF describes the intended work: inspect its title/authors/DOI after download.
