...
4. Complete post-processing.
   ...
   - For a PDF obtained through that downloader, a publisher/author source, or another authorized route, inspect its title, authors, DOI and version before attaching it. Open and validate the intended Ebib entry/database, then call `ebib-extras-attach-file` with the explicit staged path and citekey (and `t` for PDF post-processing). Return to that same entry and finish the actual `ebib-extras-process-entry` path, directly or through the headless helper. Save and verify the database and attachment. Do not hand-edit `new.bib` or select the most recent download as a substitute for this attachment step.
