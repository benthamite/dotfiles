...
#+PROPERTY: header-args :tangle ~/My Drive/dotfiles/macos/macos
:PROPERTIES:
:TOC:      ignore
:END:
...
The code blocks in this file are [[https://orgmode.org/manual/Extracting-Source-Code.html][tangled]] to generate the associated [[file:macos][macos]] shell script. Run that script to enable all these customizations in one go.
...
defaults write com.apple.speech.synthesis.general.prefs TimeAnnouncementPrefs -dict TimeAnnouncementsEnabled -bool false TimeAnnouncementsIntervalIdentifier -string "EveryHourInterval"
killall -u $(whoami) cfprefsd
echo 'Disable announce the time'
#+end_src
...
echo 'Disable screenshot floating thumbnail for immediate save'
