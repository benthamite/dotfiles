...
Run `pgrep -x Emacs` to get the PID. If multiple PIDs are returned, inspect them with `ps -p <pids> -o pid=,pcpu=,rss=,etime=,state=` and pick the best candidate, stating the selection heuristic. On macOS, `pcpu` is a long-term average since process start, so use it only as a process-selection clue, not as evidence that Emacs is currently burning CPU. If no Emacs process is found, tell the user and stop.
...
run_with_timeout 2 emacsclient -s "$SOCKET" -e \
