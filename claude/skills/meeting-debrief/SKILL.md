...
Write a `.gdoc` file next to the meeting org file so the Gemini doc is clickable from Finder:

```
/Users/pablostafforini/My Drive/Epoch/meetings/<label>/YYYY-MM-DD.gdoc
```

The file is a one-line JSON pointer that macOS's Drive handler opens in the browser. Use the Epoch account in the `email` field so clicks resolve as `pablo@epoch.ai`:

```json
{"":"WARNING! DO NOT EDIT THIS FILE! ANY CHANGES MADE WILL BE LOST!","doc_id":"<DOC_ID>","resource_key":"","email":"pablo@epoch.ai"}
```

Substitute `<DOC_ID>` with the Google Doc ID from Step 3. Do not share the doc with the personal account and do not create any Drive shortcut — just write the local file.
