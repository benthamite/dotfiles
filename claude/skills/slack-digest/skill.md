...
### 7. Ask about marking as read

After opening the buffer, ask the user: "Mark all conversations as read?"

If yes, call `conversations_mark` on every channel that had unreads (not just the ones surfaced in the digest — mark **all** channels that were fetched in step 1).
