...
(defconst org-noter-highlight-heading-regexp
  "Highlight on \\(?:page[s]?\\|p\\.\\|chapter[s]?\\|chap\\.\\) \\(.*\\)"
  "Regexp matching the highlight heading generated by `org-noter'.")

(defconst org-noter-extras-chapter-heading-regexp
  "Highlight on \\(?:chapter[s]?\\|chap\\.\\) "
  "Regexp matching chapter-based highlight headings.")
...
Replace the generic \"Highlight on page X\" generated by
`org-noter-create-skeleton' with a custom TITLE, enclose the content of the
heading in a quote block, and append a citation that includes the page number."
  (interactive)
  ...
  (let ((title (or title (read-string "Title: "))))
	...
	(let ((content (org-noter-get-annotation-contents)))
	  ...
	  (let* ((highlight-heading (org-get-heading t t t t))
		 (locator (org-noter-extras--citation-locator highlight-heading))
		 (bib-key (or (org-entry-get nil "Custom_ID" t)
			      (user-error "Cannot find Custom_ID property in any parent heading"))))
...
(defun org-noter-extras--citation-locator (heading)
  "Return the Org citation locator for annotation HEADING."
  (unless (string-match org-noter-highlight-heading-regexp heading)
    (user-error "Previous heading is not an org-noter highlight heading"))
  (let ((locator (match-string 1 heading)))
    (format "%s %s"
            (if (string-match-p org-noter-extras-chapter-heading-regexp heading)
                "chap."
              "p.")
            locator)))
...
OFFSET is the page number of the PDF (displayed in the document
viewer) minus the page number in the book or article (shown on the
printed page).  Thus, if the PDF’s page number is 220 and the
book’s page number is 210, OFFSET is 10.  The number will
typically (but need not) be positive, since the PDF page number
