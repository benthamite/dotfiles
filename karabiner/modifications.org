#+PROPERTY: header-args :tangle (print path-file-karabiner-edn)
...
1. If you have a normal keyboard, proceed to step (2). If you have a special keyboard like the Moonlandeer or the Kinesis Advantage, use their configuration software to map the three left and three right thumb keys, respectively, as follows:
     1. left: left control
     2. middle: left option
     3. right: left command
     4. left: spacebar
     5. middle: right command
     6. right: right option
...
{:des "b-mode (media controls, window sizing)"
 :rules [:b-mode
         [:u {:modi :⇧⌘⌥⌃ :key :p}] ; Rectangle, right half
         [:i {:modi :⇧⌘⌥⌃ :key :i}] ; Rectangle, maximize
         [:o {:modi :⇧⌘⌥⌃ :key :o}] ; Rectangle, center
         [:p {:modi :⇧⌘⌥⌃ :key :u}] ; Rectangle, left half
         [:j :rewind]
