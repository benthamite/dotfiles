...
| Position     | Keycode         | QMK equivalent |
|--------------+-----------------+----------------|
| Left outer   | =left_control=  | =KC_LCTL=      |
| Left middle  | =left_option=   | =KC_LALT=      |
| Left inner   | =left_command=  | =KC_LGUI=      |
| Right inner  | =spacebar=      | =KC_SPC=       |
| Right middle | =right_command= | =KC_RGUI=      |
| Right outer  | =right_option=  | =KC_RALT=      |
...
If you have a normal keyboard (no programmable thumb keys), skip to step 2.
...
| position     | keycode to send | QMK equivalent |
|--------------+-----------------+----------------|
| left outer   | ~left_control~    | ~KC_LCTL~        |
| left middle  | ~left_option~     | ~KC_LALT~        |
| left inner   | ~left_command~    | ~KC_LGUI~        |
...
*For QMK-based keyboards* (e.g. Corne, Moonlander):
...
Once flashed, verify the keyboard appears in Karabiner-Elements > Devices and that its vendor ID matches the ~:devices~ section of this config.
...
- [[#introduction][Introduction]]
  ...
  - [[#note-on-editing][Note on editing]]
- [[#main][Main]]
...
| =left_option=   | =RET= (toggle windows)          | =S-= | Shift          |
| =left_command=  | Switch to last window (=C-H-0=) | =H-= | Hyper          |
| =spacebar=      | =Cmd-Tab= (switch apps)         | =C-= | Control        |
| =right_command= | Spacebar                        | =A-= | Alt (in combos)|
...
Some simlayers additionally use =s-= as a variant modifier. For instance, in =J= (deletion), adding =s-= (via =right_option=) triggers "delete and copy" variants: =J= + =s= sends =C-H-M-s= (delete char backward), but =J= + =right_option= + =s= sends =C-H-M-s-s= (delete char backward and copy to kill ring).

The remaining simlayers (=F=, =K=, =M=, =P=, =Q=, =V=, =Y=, =;=) don't use Emacs modifier prefixes---they insert characters directly, trigger Keyboard Maestro macros, or control the mouse.

**** Outside Emacs

| Thumb key       | Tap                               | Hold    |
|-----------------+-----------------------------------+---------|
| =left_control=  | =Escape=                          | =Opt=   |
| =left_option=   | =Enter= (toggle tabs)             | =Shift= |
| =left_command=  | Switch to last tab (=Cmd-Ctrl-0=) | =Cmd=   |
| =spacebar=      | =Cmd-Tab= (switch apps)           | =Ctrl=  |
| =right_command= | Spacebar                          | ---     |
| =right_option=  | =Tab=                             | =Opt=   |

The =spacebar= key also has a combination effect outside Emacs: pressing =left_command= + =spacebar= triggers =Cmd-`= (move focus to next window).
...
Tapped: =C-g= (quit) in Emacs, =Escape= outside. Held: =M-= (Meta) in Emacs, =Option= outside. Two special rules intercept =Cmd-h= and =Cmd-Opt-h= (which macOS normally uses to hide windows) and remap them to Emacs-compatible modifier chords.
...
Tapped: =Tab=. Held: =s-= (Super) in Emacs, =Option= outside. In deletion and navigation simlayers, holding =right_option= adds the =s-= modifier to create variant commands (e.g. "delete and copy" instead of "delete").
...
I have 15 simlayers, each triggered by a different home-row or common key.
...
Hold =F= to type characters from various European languages and typographic symbols (eth, thorn, sharp s, guillemets, interrobang, etc.).

[[/images/keyboard/f-mode.svg]]
...
{:des "j-mode (deletion)"
 :rules [:j-mode
         [:a {:modi :C-H-M- :key :a} :emacs] ; sexp ←
         [:!Wa {:modi :C-H-M-s- :key :a} :emacs]
         [:!WQa {:modi :A-C-H-M-s- :key :a} :emacs]
         [:s {:modi :C-H-M- :key :s} :emacs] ; char ←
         [:s :delete_or_backspace :!emacs]
         [:d {:modi :C-H-M- :key :d} :emacs] ; char →
         [:d :delete_forward :!emacs]
         [:f {:modi :C-H-M- :key :f} :emacs] ; sexp →
         [:!Wf {:modi :C-H-M-s- :key :f} :emacs]
         [:!WQf {:modi :A-C-H-M-s- :key :f} :emacs]
         [:q {:modi :C-H-M- :key :q} :emacs] ; word ←
         [:!Wq {:modi :C-H-M-s- :key :q} :emacs]
         [:!WQq {:modi :A-C-H-M-s- :key :q} :emacs]
         [:q {:modi :⌥ :key :delete_or_backspace} :!emacs] ; word ←
         [:w {:modi :C-H-M- :key :w} :emacs] ; sentence ←
         [:!Ww {:modi :C-H-M-s- :key :w} :emacs]
         [:!WQw {:modi :A-C-H-M-s- :key :w} :emacs]
         [:w {:modi :⌘ :key :delete_or_backspace} :!emacs] ; to line start
         [:e {:modi :C-H-M- :key :e} :emacs] ; sentence →
         [:!We {:modi :C-H-M-s- :key :e} :emacs]
         [:!WQe {:modi :A-C-H-M-s- :key :e} :emacs]
         [:e {:modi :⌃ :key :k} :!emacs] ; to line end
         [:r {:modi :C-H-M- :key :r} :emacs] ; word →
         [:!Wr {:modi :C-H-M-s- :key :r} :emacs]
         [:!WQr {:modi :A-C-H-M-s- :key :r} :emacs]
         [:r {:modi :⌥ :key :delete_forward} :!emacs] ; word →
         [:z {:modi :C-H-M- :key :z} :emacs] ; line ←
         [:!Wz {:modi :C-H-M-s- :key :z} :emacs]
         [:!WQz {:modi :A-C-H-M-s- :key :z} :emacs]
         [:z {:modi :⌘ :key :delete_or_backspace} :!emacs] ; to line start
         [:x {:modi :C-H-M- :key :x} :emacs] ; paragraph ←
         [:!Wx {:modi :C-H-M-s- :key :x} :emacs]
         [:!WQx {:modi :A-C-H-M-s- :key :x} :emacs]
         [:x {:modi :⌘⌥ :key :left_arrow} :!emacs] ; select to line start
         [:c {:modi :C-H-M- :key :c} :emacs] ; paragraph →
         [:!Wc {:modi :C-H-M-s- :key :c} :emacs]
         [:!WQc {:modi :A-C-H-M-s- :key :c} :emacs]
         [:c {:modi :⌘⌥ :key :right_arrow} :!emacs] ; select to line end
         [:v {:modi :C-H-M- :key :v} :emacs] ; line →
         [:!Wv {:modi :C-H-M-s- :key :v} :emacs]
         [:!WQv {:modi :A-C-H-M-s- :key :v} :emacs]
         [:v {:modi :⌘ :key :delete_forward} :!emacs] ; line →
         [:b {:modi :C-H-M- :key :b} :emacs] ; strip thing
         [:t {:modi :C-H-M- :key :t} :emacs] ; beginning of buffer
         [:t :home :!emacs]
         [:g {:modi :C-H-M- :key :g} :emacs] ; append next kill
         [:g :end :!emacs]
...
Hold =K= in Chrome to trigger [[https://www.keyboardmaestro.com/][Keyboard Maestro]] macros for the currently active website. Each letter maps to a different action, customized per site.
...
Hold =M= for mathematical operators: === (=e=), =+= (=p=), =≠= (=q=), =÷= (=d=), =±= (=o=), =×= (=x=), =≤= (=,=), =≥= (=.=), =≈= (=a=), =∞= (=i=), =−= (=s=), =·= (=t=), =°= (=g=), =√= (=r=), =π= (=c=).

Some of these characters (===, =+=, =≠=, =÷=, =±=, =≤=, =≥=, =°=, =·=) have native Option+key shortcuts on the ABC Extended input source. The rest (=×=, =≈=, =∞=, =−=, =√=, =π=) are inserted via a shell command that copies the character to the clipboard and pastes it with =⌘V=. This means that using them will overwrite whatever is currently on the clipboard.
...
Hold =Q= + press a letter to open an application. 20+ applications are mapped. The letter choices are mnemonic where possible (e.g. =e= for Emacs, =f= for Firefox, =t= for Terminal).
...
Hold =X= + any letter to trigger [[https://github.com/abo-abo/avy][Avy]] jump commands in Emacs (this simlayer is Emacs-only). Each key maps to a unique Avy command via the =C-H-s-= modifier prefix, providing instant, keyboard-driven navigation to any visible position in the buffer.
...
{:des "x-mode (avy)"
 :rules [:x-mode
         ...
         [:f [{:modi :C-H-s- :key :f}]]
         [:g [{:modi :C-H-s- :key :g}]]
         [:h [{:modi :C-H-s- :key :h}]] ; contract
         [:i [{:modi :C-H-s- :key :i}]]
         [:j [{:modi :C-H-s- :key :j}]] ; char ←
         [:k [{:modi :C-H-s- :key :k}]] ; word ↑
         [:l [{:modi :C-H-s- :key :l}]] ; word ↓
         [:m [{:modi :C-H-s- :key :m}]] ; line ↑
         [:n [{:modi :C-H-s- :key :n}]] ; expand
         [:o [{:modi :C-H-s- :key :o}]]
         [:p [{:modi :C-H-s- :key :p}]] ; word in line →
         [:q [{:modi :C-H-s- :key :q}]]
         [:r [{:modi :C-H-s- :key :r}]]
         [:s [{:modi :C-H-s- :key :s}]]
         [:t [{:modi :C-H-s- :key :t}]]
         [:u [{:modi :C-H-s- :key :u}]] ; word in line ←
         [:v [{:modi :C-H-s- :key :v}]]
         [:w [{:modi :C-H-s- :key :w}]]
         [:y [{:modi :C-H-s- :key :y}]]
         [:z [{:modi :C-H-s- :key :z}]]
         [:semicolon [{:modi :C-H-s- :key :semicolon}]] ; char →
         [:comma [{:modi :C-H-s- :key :comma}]] ; EOL ↑
         [:period [{:modi :C-H-s- :key :period}]] ; EOL ↓
...
Hold =Y= to move the mouse cursor with the keyboard (=a=/=f= for left/right, =s=/=d= for up/down, =q=/=r= and =w=/=e= for larger jumps) or take screenshots (=z= for full screen, =c= for selected area, =v= to copy selected area to clipboard). =Enter= and =Space= act as left and right click.
...
Beyond the arrow keys, =y=/=h= provide page up/down and =m=/=/= provide home/end. In Emacs, the keys trigger custom navigation commands via the =A-C-s-= modifier prefix, enabling more granular movement (by paragraph, sentence, defun, etc.). The =##= prefix on some keys allows shift to be held for selection.
...
{:des "z-mode (navigation)"
 :rules [:z-mode
         [:b {:modi :A-C-s- :key :b} :emacs] ; other buffer
         [:c {:modi :A-C-s- :key :c}]
         [:d {:modi :A-H-M-s- :key :d} :emacs] ; sexp →
         [:e {:modi :A-C-s- :key :e} :emacs] ; sexp ←
         [:f [{:modi :⌘⌃ :key :n} {:modi :⌘⌃ :key :h}] :chrome] ; Google Docs next heading
         [:f {:modi :A-C-s- :key :f}] ; next section
         [:g {:modi :A-C-s- :key :g} :emacs] ; scroll other ↓
         [:h {:modi :A-C-s- :key :h} :emacs] ; page ↓
         [:##h :page_down]
         [:i {:modi :A-C-s- :key :i} :emacs] ; sentence ←
         [:##i {:modi :⌥ :key :up_arrow}] ; paragraph ←
         [:j :left_arrow]
         [:##j :left_arrow]
         [:k :up_arrow]
         [:##k :up_arrow]
         [:l :down_arrow]
         [:##l :down_arrow]
         [:m {:modi :A-C-s- :key :m} :emacs] ; line start
         [:##m {:modi :⌘ :key :left_arrow}] ; line start
         [:n {:modi :A-C-s- :key :n} :emacs] ; first heading
         [:o {:modi :A-C-s- :key :o} :emacs] ; sentence →
         [:##o {:modi :⌥ :key :down_arrow}] ; paragraph →
         [:p {:modi :A-C-s- :key :p} :emacs] ; word →
         [:##p {:modi :⌥ :key :right_arrow}] ; word →
         [:r [{:modi :⌘⌃ :key :p} {:modi :⌘⌃ :key :h}] :chrome] ; Google Docs previous heading
         [:r {:modi :A-C-s- :key :r} :emacs] ; prev section
         [:s {:modi :A-C-s- :key :s} :emacs] ; defun end
         [:t {:modi :A-C-s- :key :t} :emacs] ; scroll other ↑
         [:u {:modi :A-C-s- :key :u} :emacs] ; word ←
         [:##u {:modi :⌥ :key :left_arrow}] ; word ←
         [:v {:modi :A-C-s- :key :v} :emacs]
         [:w {:modi :A-C-s- :key :w} :emacs] ; defun start
         [:x {:modi :A-C-s- :key :x}]
         [:y {:modi :A-C-s- :key :y} :emacs] ; page ↑
         [:##y :page_up]
         [:spacebar {:modi :A-C-s- :key :spacebar} :emacs] ; alt buffer
         [:spacebar {:modi :⌘ :key :up_arrow} :chrome] ; in Chrome, move to top
         [:spacebar :home]
         [:comma {:modi :A-C-s- :key :comma} :emacs] ; paragraph ←
         [:##comma {:modi :⌘ :key :up_arrow}] ; doc start
         [:period {:modi :A-C-s- :key :period} :emacs] ; paragraph →
         [:##period {:modi :⌘ :key :down_arrow}] ; doc end
         [:right_command {:modi :A-C-s- :key :tab} :emacs] ; doc end
         [:right_command {:modi :⌘ :key :down_arrow} :chrome] ; in Chrome, move to bottom
         [:right_command :end]
         [:semicolon :right_arrow]
         [:##semicolon :right_arrow]
         [:slash {:modi :A-C-s- :key :slash} :emacs] ; line end
         [:##slash {:modi :⌘ :key :right_arrow}] ; line end
...
Hold =,= + a letter to trigger transposition commands in Emacs (transpose characters, words, lines, sentences, paragraphs, sexps, etc.) via the =A-H-M-= modifier prefix. This simlayer is Emacs-only.
...
{:des "comma-mode (transposition)"
 :rules [:comma-mode
         [:a {:modi :A-H-M- :key :a}] ; sexps ←
         [:b {:modi :A-H-M- :key :b}]
         [:c {:modi :A-H-M- :key :c}] ; paragraphs
         [:d {:modi :A-H-M- :key :d}] ; chars
         [:e {:modi :A-H-M- :key :e}] ; sentences
         [:f {:modi :A-H-M- :key :f}] ; sexps
         [:g {:modi :A-H-M- :key :g}]
         [:h {:modi :A-H-M- :key :h}]
         [:i {:modi :A-H-M- :key :i}]
         [:j {:modi :A-H-M- :key :j}]
         ...
         [:o {:modi :A-H-M- :key :o}]
         [:p {:modi :A-H-M- :key :p}]
         [:q {:modi :A-H-M- :key :q}] ; words ←
         [:r {:modi :A-H-M- :key :r}] ; words
         [:s {:modi :A-H-M- :key :s}] ; chars ←
         [:t {:modi :A-H-M- :key :t}] ; org element
         [:u {:modi :A-H-M- :key :u}]
         [:v {:modi :A-H-M- :key :v}] ; lines
         [:w {:modi :A-H-M- :key :w}] ; sentences ←
         [:x {:modi :A-H-M- :key :x}] ; paragraphs ←
         [:y {:modi :A-H-M- :key :y}]
         [:z {:modi :A-H-M- :key :z}] ; lines ←
...
Hold =.= + a letter to trigger text manipulation commands (sorting, aligning, casing, commenting, etc.) via the =A-C-H-= modifier prefix.
...
{:des "period-mode (manipulation)"
 :rules [:period-mode
         [:a {:modi :A-C-H- :key :a}] ; copy sexp
         [:b {:modi :A-C-H- :key :b}]
         [:c {:modi :A-C-H- :key :c}]
         [:d {:modi :A-C-H- :key :d}]
         [:e {:modi :A-C-H- :key :e}]
         [:f {:modi :A-C-H- :key :f}] ; delete sexp
         [:g {:modi :A-C-H- :key :g}]
         [:h {:modi :A-C-H- :key :h}]
         [:i {:modi :A-C-H- :key :i}]
         [:j {:modi :A-C-H- :key :j}] ; mark sexp
         [:k {:modi :A-C-H- :key :k}] ; mark around
         [:l {:modi :A-C-H- :key :l}]
         [:m {:modi :A-C-H- :key :m}]
         [:n {:modi :A-C-H- :key :n}]
         [:o {:modi :A-C-H- :key :o}]
         ...
         [:r {:modi :A-C-H- :key :r}]
         [:s {:modi :A-C-H- :key :s}]
         [:t {:modi :A-C-H- :key :t}] ; copy section
         [:u {:modi :A-C-H- :key :u}]
         [:v {:modi :A-C-H- :key :v}]
         [:w {:modi :A-C-H- :key :w}]
         [:x {:modi :A-C-H- :key :x}]
...
Hold =/= in Emacs to access Org-mode commands via the =C-H-M-s-= modifier prefix. Outside Emacs, the same keys trigger [[https://www.keyboardmaestro.com/][Keyboard Maestro]] macros specific to the active application.

[[/images/keyboard/slash-mode-emacs.svg]]

[[/images/keyboard/slash-mode-macos.svg]]

#+begin_src clojure
{:des "slash simlyaer → org-mode (Emacs) | app-specific macros (outside Emacs)"
 :rules [:slash-mode
         [:a {:modi :C-H-M-s- :key :a} :emacs] ; promote heading
         [:b {:modi :C-H-M-s- :key :b} :emacs]
         [:c {:modi :C-H-M-s- :key :c} :emacs] ; shift ↓
         [:d {:modi :C-H-M-s- :key :d} :emacs] ; move subtree ↓
         [:e {:modi :C-H-M-s- :key :e} :emacs] ; shift meta ↓
         [:f {:modi :C-H-M-s- :key :f} :emacs] ; demote heading
         ...
         [:q {:modi :C-H-M-s- :key :q} :emacs] ; promote subtree
         [:r {:modi :C-H-M-s- :key :r} :emacs] ; demote subtree
         [:s {:modi :C-H-M-s- :key :s} :emacs] ; move subtree ↑
         [:t {:modi :C-H-M-s- :key :t} :emacs]
         [:u {:modi :C-H-M-s- :key :u} :emacs]
         [:v {:modi :C-H-M-s- :key :v} :emacs] ; shift →
         [:w {:modi :C-H-M-s- :key :w} :emacs] ; shift meta ↑
         [:z {:modi :C-H-M-s- :key :z} :emacs] ; shift ←
         [:x {:modi :C-H-M-s- :key :x} :emacs] ; shift ↑
