...
- Implemented reload cooldown mechanism for auto-reload-database to prevent excessive reloading
- Refactored auto-reload callback system with improved error handling and file notification management
...
- Significantly improved =gptel-extras-import-chatgpt-conversations= function with better error handling and progress reporting
- Added support for including archived conversations with prefix argument
- Enhanced conversation filtering and processing with proper JSON handling
- Added direct links to ChatGPT conversations in imported files
- Refactored import functionality into smaller, focused functions for better maintainability

*** MCP Tool Integration
- Added comprehensive filesystem tools for AI interactions: =edit_file=, =apply_diff=, and =replace_file_contents=
- Enhanced tool system with better categorization and documentation

** Package Updates

*** Aidermacs
- Fixed compatibility issues by removing problematic hooks related to markdown syntax highlighting

*** Org Percentile
- Temporarily disabled package
...
** embark
- Added a keybinding =H-c= in the =embark-file-map= to invoke =files-extras-copy-contents=.
