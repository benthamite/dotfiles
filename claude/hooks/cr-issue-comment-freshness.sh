#!/usr/bin/env bash
# PreToolUse guard: block posting a GitHub issue comment to the reasoning-tasks repo
...
# by sync-reasoning-tasks-worktree.sh) as a PreToolUse hook on Bash. When the command
# is a comment-post to trajectory-labs-pbc/reasoning-tasks, fetch the newest comment on
# the target issue; allow only if ~/.cache/cr-freshness/reasoning-tasks-issue-<N>
...
#   mkdir -p ~/.cache/cr-freshness && echo <id> > ~/.cache/cr-freshness/reasoning-tasks-issue-<N>
...
# Only comment-POSTS to the reasoning-tasks repo. Two shapes:
#   gh issue comment <N> --repo trajectory-labs-pbc/reasoning-tasks ...
#   gh api [-X POST] repos/trajectory-labs-pbc/reasoning-tasks/issues/<N>/comments -f body=...
issue=""
case "$cmd" in
...
ack_file="$CACHE_DIR/reasoning-tasks-issue-$issue"
