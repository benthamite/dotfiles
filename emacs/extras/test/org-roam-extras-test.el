...
(ert-deftest org-roam-extras-test-recent-filters-excluded ()
  ...
  (skip-unless (featurep 'org-roam))
  (let ((org-roam-file-exclude-regexp "excluded")
        (rows `(("/tmp/roam/good.org" ,(current-time))
                ("/tmp/roam/excluded-file.org" ,(current-time))
                ("/tmp/roam/another.org" ,(current-time)))))
...
(ert-deftest org-roam-extras-test-recent-keeps-directory-with-spaces-intact ()
  "Return DB file paths with spaces intact."
  (skip-unless (featurep 'org-roam))
  (let ((org-roam-file-exclude-regexp "NOMATCH")
        (rows `(("/tmp/My Drive/roam/a.org" ,(current-time)))))
    (cl-letf (((symbol-function 'org-roam-db-query)
               (lambda (_query) rows)))
      (should (equal (org-roam-extras-recent 2)
                     '("/tmp/My Drive/roam/a.org"))))))

(ert-deftest org-roam-extras-test-recent-limit-under ()
  "Return the list when its length is under LIMIT."
  (skip-unless (featurep 'org-roam))
  (let ((org-roam-file-exclude-regexp "NOMATCH")
        (rows `(("/tmp/roam/a.org" ,(current-time))
                ("/tmp/roam/b.org" ,(current-time)))))
    (cl-letf (((symbol-function 'org-roam-db-query)
               (lambda (_query) rows)))
...
(ert-deftest org-roam-extras-test-recent-stops-at-old-files ()
  "Ignore files older than DAYS."
  (skip-unless (featurep 'org-roam))
  (let ((org-roam-file-exclude-regexp "NOMATCH")
        (rows `(("recent.org" ,(current-time))
                ("old.org" ,(time-subtract (current-time) (days-to-time 3))))))
    (cl-letf (((symbol-function 'org-roam-db-query)
               (lambda (_query) rows)))
      (should (equal (org-roam-extras-recent 2) '("recent.org"))))))
