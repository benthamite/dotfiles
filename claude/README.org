...
reviewed local workflows retain host execution. The existing Bash dispatcher
delegates the paired routing guard for recognizable untrusted launchers and
the designated staging directory. See [[file:hooks/README.org][the hook contracts]] for coverage and
[[file:../bin/README.org][the runner contract]] for available runtimes and prerequisites.
...
Claude editing calls have one destination, so the native =Write= secret-file
exception remains scoped to that file. The [[file:../codex/hooks/README.org][Codex patch guard]] separately
classifies every destination in multi-file patches; an exempt file cannot
suppress checks on ordinary files. Claude has no =apply_patch= entrypoint.
...
its authorization. Because the target is resolved from the whole command, a
write inside a compound command (pipes, =;=, =&&=) is refused before any
repository check, including a real push chained after a =--dry-run= one;
read-only GitHub commands may be piped or chained freely.
