...
The command ~forge-extras-list-project-items-ordered~ fetches all issues and pull requests from the GitHub Project configured by ~forge-extras-project-node-id~.
