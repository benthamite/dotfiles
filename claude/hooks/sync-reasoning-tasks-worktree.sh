#!/bin/bash
# SessionStart hook: keep a reasoning-tasks task worktree current with origin/main.
#
# Why: reasoning-tasks is used as git worktrees (one per CR task, each on its own
...
#
# What: on every session start, if the cwd is a reasoning-tasks worktree on a branch
...
# sessions — it exits silently unless origin is the trajectory-labs-pbc/reasoning-tasks
...
#
# Manual use: `syncreasoningtasks` (zsh function) runs this with SYNC_REASONING_TASKS_VERBOSE=1.
# Design: ~/Trajectory/docs/superpowers/specs/2026-06-24-reasoning-tasks-worktree-sync-design.md
...
private_skills_dir="${SYNC_REASONING_TASKS_PRIVATE_SKILLS_DIR:-${SYNC_AGENT_C_PRIVATE_SKILLS_DIR:-$HOME/Trajectory/.claude/skills}}"
...
# Make Pablo's PRIVATE CR skills available in THIS reasoning-tasks worktree without making
...
# outside a reasoning-tasks worktree (e.g. from the umbrella root ~/Trajectory), and the
...
repair_all_worktree_keys() {
  ...
  [ "$n" -gt 0 ] && say "repaired $n reasoning-tasks worktree key symlink(s) -> canonical."
  return 0
}
repair_all_worktree_keys
...
gc_script="${SYNC_REASONING_TASKS_GC_SCRIPT:-${SYNC_AGENT_C_GC_SCRIPT:-$HOME/My Drive/dotfiles/claude/bin/cr-worktree-gc.sh}}"
...
# Self-guard: only act in the reasoning-tasks repo (silent elsewhere — fires in all sessions).
origin_url="$(git remote get-url origin 2>/dev/null || true)"
case "$origin_url" in
  *trajectory-labs-pbc/reasoning-tasks*) : ;;
  *) vsay "not a reasoning-tasks worktree — skipping."; exit 0 ;;
esac
...
canonical_key="$HOME/Trajectory/reasoning-tasks/reasoning-tasks-cr-studio/.claude/.env"
...
# to an reasoning-tasks issue whose newest comment the agent hasn't acknowledged (the
...
# reasoning-tasks session start, independent of the merge below, so the skills are present
...
# ~/Trajectory/CLAUDE.md from the ancestor directory). The cr-studio worktree
# stays pristine.
branch="$(git symbolic-ref --quiet --short HEAD 2>/dev/null || true)"
if [ -z "$branch" ]; then
  case "$(basename "$PWD")" in
    reasoning-tasks-cr-studio|agent-c-cr-studio)
...
# Local-only root agent instructions are deliberately not pushed to reasoning-tasks.
...
if [ -n "$(git status --porcelain --untracked-files=no 2>/dev/null)" ]; then
  apply_agent_context_overlay
  say "tracked uncommitted changes on $branch — skipping; commit/stash then run 'syncreasoningtasks'."
  exit 0
fi
