...
**Goal:** Move all writable Enchant runtime symlinks outside Google Drive while preserving each dictionary's existing canonical file and clearing the 16 Enchant forwarding-link failures.
...
This is program plan 11 of 12. Start only after the Uqbar wrapper plan has
preserved the zero-directory-symlink invariant and cleared both targeted Uqbar
wrapper records. Reconfirm that every remaining targeted file-representation
record belongs to one of the 16 Enchant forwarding links, and snapshot any
unrelated record by exact path and category. Execute in the real dotfiles
checkout because both the live Enchant path and the native Drive records are
acceptance surfaces.
...
- Create outside Drive: `~/.local/state/drive-workspace-migration/enchant-runtime/<timestamp>/` — mode `0700` category manifest plus 16 path journals.
...
class InstallEnchantConfigTests(unittest.TestCase):
    ...
    def run_installer(self, *, config_root=None, drive_root=None):
        ...
        return subprocess.run(
            [
                ...
                str(SCRIPT),
                "--config-root",
                str(config_root or self.config),
                "--drive-root",
                str(drive_root or self.drive),
                "--dotfiles-root",
                str(self.dotfiles),
                "--repos-root",
                str(self.repos),
    ...
    def test_refuses_direct_config_root_under_drive_before_creation(self):
        config = self.drive / "runtime" / "enchant"
        result = self.run_installer(config_root=config)
        self.assertEqual(result.returncode, 1)
        self.assertIn("config root resolves inside Google Drive", result.stderr)
        self.assertFalse(self.drive.exists())

    def test_refuses_symlink_mediated_config_root_under_drive(self):
        self.drive.mkdir()
        alias = self.root / "drive-alias"
        alias.symlink_to(self.drive, target_is_directory=True)
        config = alias / "enchant"
        result = self.run_installer(config_root=config)
        self.assertEqual(result.returncode, 1)
        self.assertIn("config root resolves inside Google Drive", result.stderr)
        self.assertFalse((self.drive / "enchant").exists())
    ...
    @unittest.skipUnless(
        shutil.which("enchant-2") and shutil.which("enchant-lsmod-2"),
        "Enchant command-line tools not installed",
    )
    def test_disposable_dictionary_read_and_write_through_runtime_link(self):
        dictionaries = subprocess.run(
            ["enchant-lsmod-2", "-list-dicts"],
            capture_output=True,
            text=True,
        )
        self.assertEqual(dictionaries.returncode, 0, dictionaries.stderr)
        available = {
            line.split()[0]
            for line in dictionaries.stdout.splitlines()
            if line.split()
        }
        if "en" not in available:
            self.skipTest("English Enchant provider is not installed")
        result = self.run_installer()
        self.assertEqual(result.returncode, 0, result.stderr)
...
def validate_external_config(config_root: Path, drive_root: Path) -> None:
    lexical_drive = lexical_absolute(drive_root)
    lexical_config = lexical_absolute(config_root)
    resolved_drive = lexical_drive.resolve(strict=False)
    resolved_config = lexical_config.resolve(strict=False)
    if is_within(lexical_config, lexical_drive) or is_within(
        resolved_config,
        resolved_drive,
    ):
        raise InstallError(
            f"config root resolves inside Google Drive: {config_root}"
        )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--drive-root",
        type=lexical_absolute,
        default=lexical_absolute("~/My Drive"),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        validate_external_config(args.config_root, args.drive_root)
...
Expected: eleven passing tests. The Enchant integration test should run on
this machine; a skip is acceptable only when the command-line tools are absent
or `enchant-lsmod-2 -list-dicts` succeeds and reports no English provider.
...
- [ ] **Step 3: Commit the installer contract before live path mutation**
...
git commit -m "enchant: install runtime links outside Drive"
~~~

Require exactly those three staged paths, one local dotfiles commit, and no
push. This commit contains no forwarding-link deletion and therefore does not
need the later path transaction to be rolled back.
...
Capture each of the 16 forwarding links separately with `drive-workspace
capture-path --final-type absent`, run `record-path-cloud`, and append its path
journal, link text, Git mode, canonical target hash, native baseline, and one of
these authoritative cloud-preimage states: the exact pre-existing object
ID/parent when one exists, or `preexisting_cloud_object: absent` when the
exact-parent lookup finds none. Do not invent IDs for Drive-rejected links;
more than one candidate is a blocker. Require all 16 records before installing
...
### Task 5: Remove the Drive-side forwarding links without committing yet
...
- [ ] **Step 1: Pause, confirm all 16 journals, and move the tracked symlinks to Trash**

Pause Drive and run `drive-workspace confirm-path-paused --journal JOURNAL`
for every one of the 16 forwarding-link journals. Require all confirmations to
be fresh. While paused, re-read all journals, resolve and inspect the live link
texts and Git object modes, and require mode `120000`, the exact canonical
target hash, and the authoritative cloud-preimage state to match for all 16.
Then pass the 16 explicit paths to `trash`; do not use a wildcard that could
include the three regular canonical dotfiles files. While Drive remains paused,
require all 16 paths absent, the three canonical dotfiles files unchanged, and
the external runtime still complete. On any failure, run `rollback-path-local`
for each link already moved while paused and require every untouched link still
matches its journal; then require all 16 exact links and clean baseline Git
state, resume, and stop. On success, resume Drive before runtime, cloud, or
native verification.
...
- [ ] **Step 3: Review only the uncommitted forwarding-link deletion**

Run:

~~~bash
git diff --name-only -- enchant
git diff --summary -- enchant
git diff --check -- enchant
...
- the committed installer mode is `100755`;
- the committed test, README, Polish files, and ordering file modes are
  `100644`;
- all 16 deleted links remain in the index until the path transaction closes,
  but appear as the only unstaged Enchant deletions;
- unrelated concurrent changes and plan files are not staged.

- [ ] **Step 4: Run final repository verification without committing the deletions**
...
Expected: all checks pass. Do not stage or commit the 16 deletions until Task 6
closes their local, cloud, and native transactions.

### Task 6: Prove all Enchant path failures clear

**Files:**

- Verify live Google Drive state and post-restart logs.

- [ ] **Step 1: Require all 16 Enchant records to clear**

Record a UTC restart timestamp, gracefully quit and relaunch Google Drive, and
wait for settling. Inspect the native menu-bar error panel and require no
record for any removed `dotfiles/enchant` forwarding-link path and no record
for the external runtime links. Search only log entries after the restart for
`UNSUPPORTED` plus any removed `dotfiles/enchant` path; require no new match.
Require the unrelated comparison set to remain unchanged.

Run `verify-path-local` and `verify-path-cloud` for all 16 transactions and
append each native transition to the fsynced category manifest. If any runtime,
Git, cloud, or native check fails, pause Drive and obtain a fresh successful
`confirm-path-paused` record for all 16 journals. For every journal with
pre-existing object IDs, run `restore-path-cloud` for only those exact IDs. For
every journal with `preexisting_cloud_object: absent`, skip cloud restore and
require absence at the exact parent. Run `rollback-path-local` for all 16 while
paused, resume, and require all exact link texts, modes, clean baseline Git
state, authoritative cloud-preimage states, and native baseline records before
stopping. The external runtime may remain because it is outside Drive and
contains no unique data.

Only after all 16 path-level checks pass, run:
...
git diff --cached --check
git commit -m "enchant: remove Drive forwarding links"
~~~

Require the staged paths to be exactly the 16 deleted links, one local commit,
no push, and final `verify-path-local` results recording the committed absence
of all 16 paths.

- [ ] **Step 2: Run a second fresh restart**

Restart Drive again, wait for settling, and require all 16 Enchant paths to
remain absent from the native panel. Require no post-restart directory-symlink
`PARTIAL_RESULTS` in the native panel or targeted logs; any such entry breaks
the global zero-symlink prerequisite and stops completion. The final program
plan owns the overall zero-error acceptance gate.
...
These checks close only the Enchant category. The original Drive-error task
may be reported complete only after program plan 12 passes its independent
global verification.
