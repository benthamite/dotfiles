...
contains_protected_guard_path() {
    echo "$COMMAND" | grep -qE '(agents/github-write-allowlist\.txt|codex/(hooks/block-github-write-command\.sh|hooks/block-github-guard-edit\.sh|hooks/lib-codex-hook-json\.sh|hooks/lib-codex-paths\.sh|hooks/lib-repo-root\.sh|hooks\.json)|claude/(hooks/block-github-write-command\.sh|hooks/block-github-guard-edit\.sh|hooks/pretooluse-bash\.sh)|\.codex/hooks\.json|\.claude/settings\.json)'
}
...
git_push_command_p() {
    ...
    printf '%s' "$COMMAND" | grep -qE "$git_re"
}
...
if echo "$COMMAND" | grep -qE '(^|[[:space:];|&])gh[[:space:]]+release[[:space:]]+(create|delete|delete-asset|edit|upload)\b'; then
...
if echo "$COMMAND" | grep -qE '(^|[[:space:];|&])gh[[:space:]]+repo[[:space:]]+(create|delete|edit|rename|archive|unarchive|sync)\b'; then
    ...
    if [ -z "$repo" ] && echo "$COMMAND" | grep -qE '(^|[[:space:];|&])gh[[:space:]]+repo[[:space:]]+create\b'; then
...
if echo "$COMMAND" | grep -qE '(^|[[:space:];|&])gh[[:space:]]+gist[[:space:]]+(create|delete|edit|rename)\b'; then
    deny "gh gist write operation" "Gists are not repo-scoped, so the repo allowlist cannot authorize them."
fi

if echo "$COMMAND" | grep -qE '(^|[[:space:];|&])gh[[:space:]]+(cache[[:space:]]+delete|discussion[[:space:]]+(comment|create|edit)|repo[[:space:]]+(autolink[[:space:]]+(create|delete)|deploy-key[[:space:]]+(add|delete)))\b'; then
    require_allowed_repo "gh repository write operation" "$(target_repo_for_gh)"
fi

if echo "$COMMAND" | grep -qE '(^|[[:space:];|&])gh[[:space:]]+(agent-task[[:space:]]+create|label[[:space:]]+clone)\b'; then
    require_allowed_repo "gh repository write operation" "$(target_repo_for_gh)"
fi
